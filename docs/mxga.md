...
- 关键词和手动屏蔽账号可选多端同步；读取和写入均需要同一同步密钥，离线增删和并发修改会合并。
...
多端同步为可选功能。在各设备的“设置 → 个人规则同步”填写同一同步密钥，点击“保存并同步”，即可一起同步关键词、屏蔽账号和 cobalt 地址/API Key。cobalt 不需要额外开关或口令；保存、修改或清空视频下载设置都会同步，断开后仅保存在本机。自建服务的部署与客户端地址配置见 [同步服务说明](../services/mxga-sync/README.md)。


安全边界：

- 本地过滤不上传页面内容、当前 X 身份或命中结果；只有用户显式启用多端同步后，关键词、手动屏蔽账号和 cobalt 配置才会上传到需要密钥访问的同步服务。视频下载仅按下述规则将帖子链接交给 cobalt 网页或配置的服务。
- 不调用 X 私有接口，不执行 X 原生静音或拉黑。
- 点击“下载视频”后，未配置时通过 URL 片段将帖子链接交给 cobalt 网页，验证与保存由网站处理；已配置时向自建 API 发送链接，不发送 X cookie。自建服务失败或配置读取失败时不会自动切换到公共网站。为支持自定义实例，metadata 增加 `@connect *`；脚本管理器可能要求确认目标域名的访问权限。API Key 用于访问配置的 API；连接个人规则同步后，也会通过 HTTPS 随配置保存到同步服务，下载链接不附加该密钥。服务需支持 HTTPS 和 cobalt 当前 JSON API；页面兜底只暂存至多 100 条推文的媒体地址，10 分钟过期，不持久化、不读取或上传 X Cookie，不自行调用 X 内部接口；安装后需刷新 X 页面以启动采集。仅在用户点击下载且 cobalt 解析失败后使用缓存；取消不会触发兜底。
